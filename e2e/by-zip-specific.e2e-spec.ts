...
import { SeattleOpenDataSpringAngularByZipSpecificPage } from './by-zip-specific.po';
import { Helpers } from './helpers';

describe('seattle-open-data-spring-angular-frontend App', () => {
    let page: SeattleOpenDataSpringAngularByZipSpecificPage;
    let helpers: Helpers;

    beforeEach(() => {
        page = new SeattleOpenDataSpringAngularByZipSpecificPage();
        helpers = new Helpers();
    });
    ...
    it('should display first graph header text', () => {
        ...
        expect(page.getFirstGraphHeaderText()).toEqual('Calls for zip code "98104" from ' + range.startDate + ' to ' + range.endDate);
    });

    it('should display second graph header text', () => {
        page.navigateTo();
        let range = helpers.getFormattedDatesForLastNDays(30);
        expect(page.getSecondGraphHeaderText()).toEqual('Calls breakdown by Type for zip code "98104" from ' + range.startDate + ' to ' + range.endDate);
    });
    ...
    it('should have correct link to hourly breakdown page', () => {
        ...
        expect(page.getLinkBeforeFirstGraphHeaderHref()).toEqual('http://localhost:49152/hourly/by-zip/98104');
    });

    it('should display correct Title', () => {
        page.navigateTo();
        browser.getTitle().then(function (webpagetitle) {
            expect(webpagetitle).toEqual('DEV Seattle Open Data 911 Calls | By Zip - 98104');
        });
    });
});