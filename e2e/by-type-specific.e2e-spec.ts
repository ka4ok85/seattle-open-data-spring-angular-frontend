...
import { SeattleOpenDataSpringAngularByTypeSpecificPage } from './by-type-specific.po';
import { Helpers } from './helpers';

describe('seattle-open-data-spring-angular-frontend App', () => {
    let page: SeattleOpenDataSpringAngularByTypeSpecificPage;
    let helpers: Helpers;

    beforeEach(() => {
        page = new SeattleOpenDataSpringAngularByTypeSpecificPage();
        helpers = new Helpers();
    });
    ...
    it('should display first graph header text', () => {
        ...
        expect(page.getFirstGraphHeaderText()).toEqual('"Aid Response" calls from ' + range.startDate + ' to ' + range.endDate);
    });

    it('should display second graph header text', () => {
        page.navigateTo();
        let range = helpers.getFormattedDatesForLastNDays(30);
        expect(page.getSecondGraphHeaderText()).toEqual('"Aid Response" calls breakdown by zip code from ' + range.startDate + ' to ' + range.endDate);
    });
    ...
    it('should display link text to hourly breakdown page', () => {
        page.navigateTo();

        expect(page.getLinkBeforeFirstGraphHeaderText()).toEqual('See hourly breakdown »');
    });

    it('should have correct link to hourly breakdown page', () => {
        page.navigateTo();

        expect(page.getLinkBeforeFirstGraphHeaderHref()).toEqual('http://localhost:49152/hourly/by-type/Aid%20Response');
    });

    it('should display correct Title', () => {
        page.navigateTo();
        browser.getTitle().then(function (webpagetitle) {
            expect(webpagetitle).toEqual('DEV Seattle Open Data 911 Calls | By Type - Aid Response');
        });
    });
