...
import { SeattleOpenDataSpringAngularHourlyByTypePage } from './hourly-by-type.po';
import { Helpers } from './helpers';

describe('seattle-open-data-spring-angular-frontend App', () => {
    let page: SeattleOpenDataSpringAngularHourlyByTypePage;
    let helpers: Helpers;

    beforeEach(() => {
        page = new SeattleOpenDataSpringAngularHourlyByTypePage();
        helpers = new Helpers();
    });
    ...
    it('should display first graph header text', () => {
        ...
        expect(page.getFirstGraphHeaderText()).toEqual('"Aid Response" calls hourly breakdown from ' + range.startDate + ' to ' + range.endDate);
    });
    ...
    it('should display correct Title', () => {
        ...
        browser.getTitle().then(function (webpagetitle) {
            expect(webpagetitle).toEqual('DEV Seattle Open Data 911 Calls | Hourly By Type - Aid Response');
        });
    });
