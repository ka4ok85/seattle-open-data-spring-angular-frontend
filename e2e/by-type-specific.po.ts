...
export class SeattleOpenDataSpringAngularByTypeSpecificPage {
    navigateTo() {
        return browser.get('/by-type/Aid%20Response');
    }

    /* Sections */
    getMainSection() {
        return element(by.css('app-root ng-component section#by-type-specific'));
    }
    ...
    getLinkBeforeFirstGraphHeaderText() {
        return this.getMainBodyFirstRowLeftSection().all(by.css('h3')).first().getText();
    }

    getLinkBeforeFirstGraphHeaderHref() {
        return this.getMainBodyFirstRowLeftSection().all(by.css('h3')).first().element(by.css('a')).getAttribute('href');
    }

    getFirstGraphHeaderText() {
        return this.getMainBodyFirstRowLeftSection().element(by.css('h3.text-center')).getText();
    }
