...
import { SeattleOpenDataSpringAngularByTypePage } from './by-type.po';
import { Helpers } from './helpers';

describe('seattle-open-data-spring-angular-frontend App', () => {
    let page: SeattleOpenDataSpringAngularByTypePage;
    let helpers: Helpers;

    beforeEach(() => {
        page = new SeattleOpenDataSpringAngularByTypePage();
        helpers = new Helpers();
    });
    ...
    it('should display first graph header text', () => {
        ...
        expect(page.getFirstGraphHeaderText()).toEqual('Top 5 Calls Types from ' + range.startDate + ' to ' + range.endDate);
    });

    it('should display second graph header text', () => {
        page.navigateTo();
        let range = helpers.getFormattedDatesForLastNDays(30);
        expect(page.getSecondGraphHeaderText()).toEqual('Calls breakdown by Type from ' + range.startDate + ' to ' + range.endDate);
    });
    ...
    it('should display correct Title', () => {
        ...
        browser.getTitle().then(function (webpagetitle) {
            expect(webpagetitle).toEqual('DEV Seattle Open Data 911 Calls | By Type');
        });
    });
