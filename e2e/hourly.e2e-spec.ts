...
describe('seattle-open-data-spring-angular-frontend App', () => {
    let page: SeattleOpenDataSpringAngularHourlyPage;
    let helpers: Helpers;

    beforeEach(() => {
        page = new SeattleOpenDataSpringAngularHourlyPage();
        helpers = new Helpers();
    });
    ...
    it('should display first graph header text', () => {
        ...
        expect(page.getFirstGraphHeaderText()).toEqual('Calls hourly breakdown from ' + range.startDate + ' to ' + range.endDate);
    });
    ...
    it('should display correct Title', () => {
        ...
        browser.getTitle().then(function (webpagetitle) {
            expect(webpagetitle).toEqual('DEV Seattle Open Data 911 Calls | Hourly');
        });
    });
